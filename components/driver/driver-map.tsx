...
/**
 * Hito 4.0 - Versión v.3.1 (Estabilidad Android DOM)
...
const GeofenceCircle = ({ center, radius }: { center: google.maps.LatLngLiteral, radius: number }) => {
  ...
  useEffect(() => {
    ...
    const circle = new google.maps.Circle({
      ...
      strokeColor: '#2563eb', strokeOpacity: 0.5,
      strokeWeight: 2, clickable: false, zIndex: 1
    });
...
const GlowPolyline = ({ encodedPath }: { encodedPath: string }) => {
  ...
  useEffect(() => {
    ...
    const line = new google.maps.Polyline({
      path, map, strokeColor: '#2563eb', 
      strokeOpacity: 1.0, strokeWeight: 6, zIndex: 2,
      icons: [{
        icon: {
          path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW,
          fillColor: '#ffffff', fillOpacity: 1,
          strokeColor: '#ffffff', scale: 2.5
        }, offset: '50px', repeat: '100px'
      }],
    });
  ...
  return (
    <>
      {stops.map((stop, index) => {
        const isDelivered = stop.status === "DELIVERED" || stop.status === "SKIPPED" || index < currentStopIndex;
        ...
        return (
          ...
          >
            {/* FIX: Se cambia renderizado condicional por display:none para evitar colisión DOM en móviles */}
            <div 
              className="absolute -inset-2 rounded-full animate-ping bg-yellow-400/40 z-[-1]" 
              style={{ display: (isGroundTruth && !isDelivered) ? 'block' : 'none' }}
            />
            <Pin 
              background={bgColor} 
    ...
    <div className="w-full h-72 rounded-2xl overflow-hidden border-2 border-white/10 shadow-2xl relative bg-muted">
      <APIProvider apiKey={process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY || ''} libraries={['geometry']}>
        <Map 
          ...
          // ELIMINADO: reuseMaps={true} causaba el crash de DOM Node en dispositivos Android
        >
